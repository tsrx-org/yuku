// Runtime half of docs/widgets/generate-diff.mjs: both columns and the diff
// come from the generator running in this tab.
...
export default function mount(root, { cleanup }) {
  ...
  const runSide = async (id) => {
    ...
    const npmNote =
      options.strip || options.minify
        ? ' <span class="gd-caveat">strip and minify act here; the npm generate() ignores both today</span>'
        : ''
    call.innerHTML = `<code>${escapeHtml(equivalentCall(options))}</code>${npmNote}`
    return result
  }
