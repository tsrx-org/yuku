// Static docs site generator: markdown in yuku-website/ -> HTML in yuku-website/dist/.
// Plain JavaScript, no framework. Run with: node yuku-website/build.mjs
...
// yuku-website/assets/style.css is authored as one file but most of it can only ever
...
// figure that yuku-website/assets/yuku-explorers.js drives with the WebAssembly build
...
// `<!-- terminal-demo:NAME -->` embeds yuku-website/transcripts/NAME.json, written by
...
async function readHooks() {
  ...
  if (unmapped.length > 0) {
    throw new Error(`yuku-website/build.mjs has no area for hook(s): ${unmapped.join(', ')}`)
  }
...
function hookMatrixHtml(article, hooks, sourcePath) {
  ...
  const body = rows.map(([, hookCell, areaCell, roleCell], index) => {
    ...
    if (area !== hook.area) {
      throw new Error(
        `${sourcePath}: row ${index + 1} (\`${name}\`) is filed under ${area}, but yuku-website/build.mjs files it under ${hook.area}`,
      )
    }
...
// becomes the widget's seed, is rendered by yuku-website/widgets/NAME.mjs at build time
// and driven by yuku-website/assets/widgets/NAME.js in the reader's tab. Both files must
...
async function build() {
  ...
  // A widget's own rules live beside its runtime half as yuku-website/assets/widgets/NAME.css
