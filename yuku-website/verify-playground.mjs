...
// open one.
//
//   node yuku-website/verify-playground.mjs                serves yuku-website/dist locally
//   node yuku-website/verify-playground.mjs --url <origin> runs against a deployment
//
// Pages are found by what the build emitted into yuku-website/dist, not by a list here:
...
async function startServer() {
  ...
  const origin = await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => reject(new Error(`yuku-website/serve.mjs did not start: ${stderr || buffer}`)),
      15_000,
    )
    ...
    child.on('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`yuku-website/serve.mjs exited with ${code}: ${stderr}`))
    })
  })
...
async function main() {
  ...
  try {
    ...
    if (terminalRoute) {
      ...
      if (check(Boolean(recording), `${terminalRoute}: no committed transcript is named "${name}"`)) {
        for (const entry of recording.transcript) {
          ...
          check(entry.exit_code === 0, `yuku-website/transcripts: ${entry.command} is committed with exit ${entry.exit_code}`)
    ...
    // Each widget may ship yuku-website/widgets/NAME.verify.mjs exporting
  ...
  if (wasm) notes.push(`wasm: ${(wasm.size / 1024).toFixed(0)} KiB in yuku-website/dist`)
}
