// Minimal static file server for the built docs site. Run: node yuku-website/serve.mjs [port]
// Binds 127.0.0.1 explicitly so an already-taken port fails loudly instead of
// silently coexisting on another interface.
//
// It serves yuku-website/dist exactly as the deploy does: extensionless routes resolve
...
server
  ...
  .on('error', (error) => {
    if (error.code === 'EADDRINUSE') {
      console.error(`port ${requestedPort} is already in use, pass another: node yuku-website/serve.mjs <port>`)
      process.exit(1)
    }
    throw error
  })
