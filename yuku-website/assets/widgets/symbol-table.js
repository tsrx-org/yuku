// Reads names or full tables from the SemanticView returned in this tab.
...
export default function mount(root, { cleanup }) {
  ...
  const describeName = (offset) => {
    const ref = model?.references.find((candidate) => candidate.start <= offset && candidate.end > offset)
    const symbol = ref?.symbolId === null
      ? null
      : ref?.symbolId !== undefined
        ? model.symbols[ref.symbolId]
        : model?.symbols.find((candidate) => candidate.decls.some((span) => span.start <= offset && span.end > offset))
    if (ref?.symbolId === null) {
      readout.textContent = `${ref.name}: no declaration in this file, symbolId is null`
      paint(segments, [ref], 'ex-hit')
      return
    }
    if (!symbol) return
    const imported = model.imports.some((entry) => entry.symbolId === symbol.id)
    readout.textContent = imported
      ? `${symbol.name}: declared in the import`
      : `${symbol.name}: declared at line ${lineAt(symbol.decls[0].start)}`
  }
  ...
  const showResult = async (result) => {
    ...
    if (errors.length > 0) {
      const errorHtml = `<ul class="explorer-diagnostics ex-errors">${errors
        .map((error) => `<li>${escapeHtml(error.message)}</li>`)
        .join('')}</ul>`
      if (out) out.innerHTML = errorHtml
      else readout.innerHTML = errorHtml
    ...
    for (const symbol of model.symbols) {
      const imported = model.imports.some((entry) => entry.symbolId === symbol.id)
      spans.push(...symbol.decls.map((span) => ({
        ...span,
        readout: imported ? `${symbol.name}: declared in the import` : `${symbol.name}: declared at line ${lineAt(span.start)}`,
      })))
    }
    await editor.render()
    const missing = model.references.filter((ref) => ref.symbolId === null)
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const unavailable = `in-browser analyzer unavailable: ${escapeHtml(error?.message ?? String(error))}`
      if (out) out.innerHTML = `<p class="ex-note ex-unavailable">${unavailable}</p>`
      else readout.innerHTML = unavailable
  ...
  ready()
    .then(() => {
      editor = createLayeredEditor({
        ...
        onChange(value) {
          source = value
          if (reset) reset.hidden = source === seed
          runAnalysis()
        },
    ...
    .catch((error) => {
      if (out) out.innerHTML = `<p class="ex-note ex-unavailable">in-browser analyzer unavailable: ${escapeHtml(error.message)}</p>`
      else readout.textContent = `in-browser analyzer unavailable: ${error.message}`
      failWidget(root, 'in-browser analyzer unavailable', error, 'unavailable')
    })
}
