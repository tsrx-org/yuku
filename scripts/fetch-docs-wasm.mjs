...
function parseStamp(bytes, label) {
  ...
  if (stamp?.tree !== pin.srcTree || stamp?.dirty !== false || typeof stamp?.built_at !== 'string') {
...
if (!Number.isInteger(pin.sizeBytes) || pin.sizeBytes < 1) fail('wasm-pin.json has no valid sizeBytes')
...
try {
  ...
  if (error?.code !== 'ENOENT' && !String(error?.message).startsWith('fetch-docs-wasm:')) throw error
}
